...
func coalesceGlobals(dest, src map[string]interface{}) map[string]interface{} {
	...
	if destglob, ok := dest[GlobalKey]; !ok {
		...
		glog.Infof("warning: skipping globals because destination %s is not a table.", GlobalKey)
		return dg
	}
	...
	if srcglob, ok := src[GlobalKey]; !ok {
		...
		glog.Infof("warning: skipping globals because source %s is not a table.", GlobalKey)
		return dg
	}
		...
		if istable(val) {
			...
			if destv, ok := dg[key]; ok {
				if destvmap, ok := destv.(map[string]interface{}); ok {
					...
					glog.Infof("Conflict: cannot merge map onto non-map for %q. Skipping.", key)
				}
			} else {
			...
			glog.Infof("key %s is table. Skipping", key)
			continue
		}
	...
	for key, val := range src {
		if istable(val) {
			if innerdst, ok := dst[key]; !ok {
				...
				glog.Infof("warning: cannot overwrite table with non table for %s (%v)", key, val)
			}
			continue
		} else if dv, ok := dst[key]; ok && istable(dv) {
			glog.Infof("warning: destination for %s is a table. Ignoring non-table value %v", key, val)
			continue
